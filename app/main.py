...
with tab1:
    with st.sidebar:
        ...
        trabalho_idx = {"Desempleado":0,"Básico":1,"Calificado":2,"Especialista":3}[trab_sel]
        
        hab_sel = st.selectbox("Vivienda", ["Propia","Alquilada","Gratuita"])
        habitacao_val = {"Propia":"own","Alquilada":"rent","Gratuita":"free"}[hab_sel]
        
        ahorro_sel = st.selectbox("Cuenta de Ahorro", ["Bajo","Medio","Alto"])
        ahorro_val = {"Bajo":"little","Medio":"moderate","Alto":"rich"}[ahorro_sel]
        
        corr_sel = st.selectbox("Cuenta Corriente", ["Bajo","Medio","Alto"])
        corriente_val = {"Bajo":"little","Medio":"moderate","Alto":"rich"}[corr_sel]
        
        fin_sel = st.selectbox("Finalidad", ["Auto","Muebles","Electrónicos","Negocios","Educación","Reparaciones","Otros"])
        finalidade_val = {"Auto":"car","Muebles":"furniture/equipment","Electrónicos":"radio/TV","Negocios":"business","Educación":"education","Reparaciones":"repairs","Otros":"vacation/others"}[fin_sel]
    ...
    if btn:
        # DataFrame para o Scorecardpy (Nomes em Português conforme o treino)
        ...
        # Chama a inteligência do src/policy.py
        score_base = get_score(prob, score_params)
        res = apply_business_policy(score_base, trabalho_idx, habitacao_val, ahorro_val, corriente_val, monto)
        ...
        with col_res:
            ...
            st.markdown(f"<p style='text-align:center;margin-bottom:0;font-size:14px;'>Probabilidad de Incumplimiento</p><p style='text-align:center;font-size:22px;font-weight:700;'>{prob:.2%}</p>", unsafe_allow_html=True)
