...
# ============================================
# PATH E CARREGAMENTO (AJUSTADO PARA SEUS ARQUIVOS)
# ============================================
...
@st.cache_resource
def load_data():
    # Nomes conforme sua estrutura real identificada
...
# ============================================
# CSS CUSTOMIZADO (MANTENDO SEU LAYOUT ORIGINAL)
# ============================================
st.markdown("""
<style>
...
# ============================================
# TAB 1: SIMULACIÓN (COM REGRAS DE NEGÓCIO ATUALIZADAS)
# ============================================
with tab1:
    with st.sidebar:
        ...
        habitacao = {"Propia":"own","Alquilada":"rent","Gratuita":"free"}[habitacion_sel]
    ...
    if btn:
        ...
        # Odds de "Bom" (1-p / p)
        odds = (1 - prob) / prob
        score_base = int(round(offset + factor * np.log(odds)))
        
        # Ajustes de Política (Penalidades)
        penalidade_score = 0
        penalidade_limite = 1.0
        flags = []

        if trabalho == 0: penalidade_score -= 80; penalidade_limite *= 0.5; flags.append("Sin empleo")
        if habitacao == "rent": penalidade_score -= 30; penalidade_limite *= 0.85; flags.append("Vivienda alquilada")
        ...
        score_final = max(score_base + penalidade_score, 300)

        # 3. SEGMENTAÇÃO E LIMITE
        if score_final >= 700: segmento="SUPER PRIME"; limite_base=18000
        elif score_final >= 650: segmento="PRIME"; limite_base=10000
        elif score_final >= 600: segmento="STANDARD"; limite_base=5000
        elif score_final >= 520: segmento="NEAR PRIME"; limite_base=2500
        elif score_final >= 460: segmento="REVIEW"; limite_base=1000
        else: segmento="SUBPRIME"; limite_base=0

        limite = int(limite_base * penalidade_limite)
        if duracion > 48: limite = int(limite * 0.85)
        
        # 4. DECISÃO FINAL (STATUS)
        if trabalho == 0 and cuenta_corriente == "little":
            ...
            if score_final < 460: status="RECHAZADO"; icon="✖"; cor="#dc2626"; motivo="Score bajo política mínima."
            elif score_final < 520: status="EN ANÁLISIS"; icon="⚠"; cor="#facc15"; motivo="Zona intermedia de riesgo."
            else:
        ...
        with col_res:
            ...
            st.markdown(f"<div class='score' style='color:{cor};'>{score_final}</div>", unsafe_allow_html=True)
