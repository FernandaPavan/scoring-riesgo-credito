...
# Mapeamentos: Interface (Espanhol) -> Modelo (Técnico)
...
def traduzir_inputs(genero_sel, trabalho_sel, vivienda_sel, ahorro_sel, corriente_sel, finalidad_sel):
    return (
        MAP_GENERO[genero_sel], MAP_TRABAJO[trabalho_sel], MAP_VIVIENDA[vivienda_sel],
...
def montar_entrada(genero, trabalho, vivienda, ahorro, corriente, finalidad, edad, duracion, monto):
    return pd.DataFrame({
        "Genero": [genero], "Trabalho": [trabalho], "Habitacao": [vivienda],
        "Conta_poupanca": [ahorro], "Conta_corrente": [corriente],
        "Finalidade": [finalidad], "Idade": [edad], "Duracao": [duracion], "Valor_credito": [monto]
    })

def criar_faixas(X):
    X = X.copy()
    X['Faixa_Etaria'] = pd.cut(X['Idade'], bins=[19, 29, 39, 49, 59, 69, 79], include_lowest=True).astype(str)
    X['Faixa_Duracao'] = pd.cut(X['Duracao'], bins=[4, 16, 28, 40, 52, 64, 76], include_lowest=True).astype(str)
    X['Faixa_Credito'] = pd.cut(X['Valor_credito'], bins=[250, 1250, 2250, 3250, 4250, 5250, 6250], include_lowest=True).astype(str)
